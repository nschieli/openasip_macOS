...
| **Gatekeeper and downloaded binaries** | ⛔ **A BROWSER DOWNLOAD QUARANTINES EVERYTHING, AND THE ghdl CASK BELOW IS AN INSTANCE OF THIS, NOT A SEPARATE CURIOSITY.** Unsigned or ad-hoc-signed Mach-O files that carry `com.apple.quarantine` are refused by macOS, and for a command-line tool the dialog offers only "Move to Trash" — there is no "Open Anyway", and the System Settings override is **per binary**. ⚠ Archive Utility (a Finder double-click) **propagates the flag to every extracted file**; `tar -xzf` does not. Measured 2026-09-16 on macOS 26 against this project's own release tarball: 5477 of 5478 files quarantined, tools refused. ⇒ Extract downloads with `tar` from a terminal. `xattr -dr com.apple.quarantine <tree>` works only **before** you try to run anything. ⛔ **ONCE macOS HAS REFUSED A FILE, THE REFUSAL IS CACHED AGAINST THAT COPY AND CLEARING THE ATTRIBUTE DOES NOT LIFT IT** — measured 2026-09-18: a tree killed once stayed killed, while the *same bytes* `ditto`'d to a new path ran normally. ⇒ **The fix after a block is RE-EXTRACT, not `xattr`.** ⚠ This is also why a stale test tree can lie to you about a build that is actually fine. ⭐ `curl`, `gh` and `git` do not set the flag at all — which is why this is invisible to anyone testing from a terminal, and why "it works here" is not evidence about a user who clicked a link. |
