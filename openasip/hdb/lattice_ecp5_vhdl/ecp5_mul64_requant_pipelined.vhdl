...
entity ecp5_mul64_requant_pipelined is
  port (
    clk          : in  std_logic;
    rstx         : in  std_logic;
    ...
    -- Trigger port (sets opcode, 3 bits for 5 operations)
...
end entity ecp5_mul64_requant_pipelined;

architecture rtl of ecp5_mul64_requant_pipelined is

  -- Opcodes (alphabetical: emac2x32, emac4x32, mac64, mul64, requant)
  ...
  constant OP_MUL64    : std_logic_vector(2 downto 0) := "011";
  constant OP_REQUANT  : std_logic_vector(2 downto 0) := "100";
...
begin
  ...
  --
  -- Stage 2 output mux (at second clock edge after trigger):
  --   mac64:   acc + mul_result
  --   mul64:   mul_result
  --   requant: sign-extend clamped int8 to 64 bits
  ---------------------------------------------------------------------------
      ...
      if glock_in = '0' then
        if valid_s2_r = '1' then
          if op_s2_r = OP_REQUANT then
            ...
            -- acc_ch0 += f_a0*(input_a+off) + f_b0*(input_b+off)
            -- acc_ch1 += f_a1*(input_a+off) + f_b1*(input_b+off)
            result_r(31 downto 0) <= std_logic_vector(
