...
**Lattice ECP5 function units** — pipelined multipliers, a SIMD multiplier, an
ALU and register files in `openasip/hdb/lattice_ecp5_vhdl/`, and the HDBs that
use them. ⚠ **Not MIT**: CERN-OHL-S-2.0, or a commercial licence. ProGe copies
these files verbatim into a generated processor, so a design that uses them is
covered by that licence. The HDBs are listed in `openasip/hdb/LICENSE-NOTICE.md`.
